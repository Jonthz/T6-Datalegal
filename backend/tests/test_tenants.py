...
class TestTenantProvisioning:
    def test_provision_tenant_as_super_admin(self, client: TestClient, super_admin_token):
        resp = client.post(
            ...
            json={
                "tenant": {"name": "New Corp", "ruc": "1111111111001", "country": "Ecuador", "sector": "Tech"},
                "admin_user": {
    ...
    def test_provision_tenant_duplicate_ruc(self, client: TestClient, super_admin_token):
        client.post(
            ...
            json={
                ...
                "admin_user": {"email": "adm1@first.com", "password": "Secure@Pass1!", "full_name": "Admin", "role": "DPO"},
            },
            headers=auth_headers(super_admin_token),
        )
        # Try same RUC again
        resp = client.post(
            ...
            json={
                ...
                "admin_user": {"email": "adm2@second.com", "password": "Secure@Pass1!", "full_name": "Admin2", "role": "DPO"},
            },
            headers=auth_headers(super_admin_token),
        )
    ...
    def test_provision_tenant_non_super_admin_forbidden(self, client: TestClient, admin_token):
        resp = client.post(
            ...
            json={
                ...
                "admin_user": {"email": "unauth@test.com", "password": "Secure@Pass1!", "full_name": "Unauth", "role": "DPO"},
            },
            headers=auth_headers(admin_token),
        )
        assert resp.status_code == 403

    def test_list_tenants_as_super_admin(self, client: TestClient, super_admin_token, tenant_a):
...
class TestTenantIsolation:
    def test_tenant_a_cannot_see_tenant_b_users(
        self, client: TestClient, admin_token, tenant_b_user, tenant_b_token, session
    ):
    ...
    def test_tenant_b_user_sees_only_own_users(
        self, client: TestClient, tenant_b_token, tenant_b, tenant_b_user, session
    ):
