...
class TestRBACPermissions:
    ...
    def test_dept_head_cannot_access_audit(self, client: TestClient, dept_head_token):
        """Test that dept head cannot access audit behaves as expected."""
    ...
    def test_auditor_can_read_audit(self, client: TestClient, auditor_token):
        """Test that auditor can read audit behaves as expected."""
    ...
    def test_auditor_can_export_audit(self, client: TestClient, auditor_token):
        """Test that auditor can export audit behaves as expected."""
    ...
    def test_dept_head_can_read_users(self, client: TestClient, dept_head_token):
        """Test that dept head can read users behaves as expected."""
    ...
    def test_dept_head_cannot_delete_users(self, client: TestClient, dept_head_token, admin_user):
        """Test that dept head cannot delete users behaves as expected."""
        resp = client.delete(
            f"/api/v1/users/{admin_user.id}", headers=auth_headers(dept_head_token)
        )
        assert resp.status_code == 403

    def test_admin_can_create_user(self, client: TestClient, admin_token):
        """Test that admin can create user behaves as expected."""
        resp = client.post(
            "/api/v1/users",
            json={
                "email": "by_admin@test.com",
                "password": "Valid@Pass1!",
                "full_name": "By Admin",
                "role": "AUDITOR",
            },
    ...
    def test_unauthenticated_request_rejected(self, client: TestClient):
        """Test that unauthenticated request rejected behaves as expected."""
    ...
    def test_auditor_cannot_create_department(self, client: TestClient, auditor_token):
        """Test that auditor cannot create department behaves as expected."""
        resp = client.post(
            "/api/v1/departments",
    ...
    def test_dpo_can_create_department(self, client: TestClient, dpo_token):
        """Test that dpo can create department behaves as expected."""
        resp = client.post(
            "/api/v1/departments",
    ...
    def test_permission_failure_is_logged(self, client: TestClient, auditor_token, session):
        """Test that permission failure is logged behaves as expected."""
        from app.models.audit_log import AuditLog

        initial_count = (
            session.query(AuditLog).filter(AuditLog.action == "permission_check_fail").count()
        )
